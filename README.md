...
├── data/
│   ├── operators.json        事業者マスタ
│   ├── stations.json         駅マスタ（1,016駅）
│   └── lines/
│       ├── _TEMPLATE.json    新規路線の雛形
│       ├── jr-east--*.json       JR東日本 54路線
│       ├── jr-central--*.json    JR東海 3路線
│       ├── tokyo-metro--*.json   東京メトロ 9路線＋分岐線
│       └── toei--*.json          都営地下鉄 4路線
...
│   └── LINE_DETAIL.md        全173路線の記号・カラー・駅数の一覧
└── tools/
...
現在は314駅が2路線以上につながっている（東京駅は11路線、渋谷は9路線）。
